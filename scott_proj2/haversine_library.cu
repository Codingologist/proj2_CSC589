    ...
    const double *x2,const double *y2, double *dist)
{
 ...
 // used stackoverflow reference listed above
 int ind = blockIdx.x * blockDim.x + threadIdx.x; //block index
 int R = 6371;

 if (ind < size) {
    double dLat = (y2[ind] - y1[ind]) * (M_PI / 180); double dLon = (x2[ind] - x1[ind]) * (M_PI / 180);
    double y1Rad = y1[ind] * (M_PI / 180); double y2Rad = y2[ind] * (M_PI / 180);
